...
class App:
    def __init__(self, font_video=0):
        ...
        bt_canvas.pack(side=BOTTOM)
        Button(bt_canvas, text="GUARDAR", width=10, bg='light blue', command=self.guardar).pack(side=LEFT)
        Button(bt_canvas, text="COPIAR", width=10, bg='light blue', command=self.copy_info).pack(side=LEFT)
        ...
        self.canvas.pack()
        Button(self.ventana, text="CARGAR ARCHIVO", width=29, bg='goldenrod2', activebackground='red',
               command=self.abrir).pack(side=LEFT)
        ...
        Button(self.ventana, text="DETECTAR EN PANTALLA", width=29, bg='goldenrod2', activebackground='red',
               command=self.screen_shot).pack(side=RIGHT)
    ...
    def copy_info(self):
        try:
            if len(self.display.get('1.0', END)) > 1:
                pyperclip.copy(self.display.get('1.0', END))
